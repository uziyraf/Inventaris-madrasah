...
@section('content')
    <div x-data="{ 
        ...
        // Buka modal pake data asli yang dikirim dari looping blade
        openModal(className, l, p, total, studentsList) {
        ...
        <!-- Summary Card (Dinamis dari Database) -->
        <div
            class="bg-gradient-to-br from-[#207e60] to-[#155d44] rounded-sm shadow-sm p-8 mb-8 text-white flex flex-col md:flex-row justify-between items-center gap-8 relative overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <div class="flex items-baseline gap-3">
                    <h3 class="text-[48px] font-extrabold leading-none tracking-tight">{{ $totalSiswa }}</h3>
            ...
            <div class="relative z-10 flex gap-4 w-full md:w-auto">
                <div
                    class="flex-1 md:flex-none bg-white/10 rounded-sm p-5 backdrop-blur-md border border-white/20 min-w-[140px]">
                    ...
                    <p class="text-[32px] font-bold leading-none">{{ $totalLaki }}</p>
                </div>
                <div
                    class="flex-1 md:flex-none bg-white/10 rounded-sm p-5 backdrop-blur-md border border-white/20 min-w-[140px]">
                    <div class="flex items-center gap-2 mb-3">
                    ...
                    <p class="text-[32px] font-bold leading-none">{{ $totalPerempuan }}</p>
                </div>
            </div>
        </div>
        ...
        <!-- Cards Grid (Looping Otomatis) -->
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            @forelse($kelasData as $data)
                <div x-data="{ currentStudents: {{ json_encode($data['students']) }} }"
                    @click="openModal('{{ $data['nama_kelas'] }}', {{ $data['l'] }}, {{ $data['p'] }}, {{ $data['total'] }}, currentStudents)"
                    class="bg-white rounded-sm shadow-sm border border-gray-200 overflow-hidden hover:border-[#1c7b5b] transition-all hover:shadow-md group cursor-pointer">

                    <div
                        class="p-5 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center group-hover:bg-[#f0fbf7]/50 transition-colors">
                        <div class="flex items-center gap-3">
                            <div
                                class="w-8 h-8 rounded-sm bg-[#1c7b5b] text-white flex items-center justify-center font-bold text-[13px]">
                                <!-- Ambil huruf pertama atau nomor dari nama kelas -->
                                {{ substr(preg_replace('/[^a-zA-Z0-9]/', '', $data['nama_kelas']), -1) }}
                            </div>
                            <h3 class="text-[16px] font-bold text-[#1e293b]">{{ $data['nama_kelas'] }}</h3>
                        </div>
                        <span
                            class="bg-[#e2f5ec] text-[#1c7b5b] text-[11px] font-bold px-2.5 py-1 rounded-sm border border-[#1c7b5b]/10">{{ $data['total'] }}
                            Siswa</span>
                    </div>
                    ...
                    <div class="p-6">
                        <div class="flex justify-between items-center mb-4 pb-4 border-b border-gray-100 border-dashed">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-8 h-8 rounded-full bg-blue-50 flex items-center justify-center text-blue-600 border border-blue-100">
                            ...
                            <span class="text-[18px] font-bold text-blue-600">{{ $data['l'] }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-8 h-8 rounded-full bg-pink-50 flex items-center justify-center text-pink-500 border border-pink-100">
                            ...
                            <span class="text-[18px] font-bold text-gray-400">{{ $data['p'] }}</span>
                        </div>
                    </div>
                </div>
            @empty
                <div class="col-span-full bg-white p-10 text-center rounded-sm border border-gray-200 shadow-sm">
                    <p class="text-gray-500">Belum ada data siswa / santri yang didaftarkan.</p>
                </div>
            @endforelse
        </div>
        ...
        <div x-show="modalOpen" class="fixed inset-0 z-[100] flex items-center justify-center" style="display: none;"
            x-cloak>
            <!-- Backdrop -->
            <div x-show="modalOpen" x-transition.opacity class="absolute inset-0 bg-slate-900/40 backdrop-blur-sm"
            ...
            <div x-show="modalOpen" x-transition:enter="transition ease-out duration-300"
                ...
                <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 shrink-0">
                    <div>
                        ...
                        <p class="text-[12px] text-gray-500 mt-0.5"
                            x-text="`Daftar ${selectedData.total} siswa yang terdaftar pada kelas ini`"></p>
                    </div>
                ...
                <div class="overflow-y-auto flex-1 p-0">
                    <table class="w-full text-left text-sm text-gray-600">
                        <thead
                            class="text-[11px] text-gray-500 font-bold tracking-widest uppercase border-b border-gray-100 bg-white sticky top-0 z-10 shadow-sm">
                            <tr>
                        ...
                        <tbody class="divide-y divide-gray-100 bg-white">
                            <template x-for="(student, index) in students" :key="index">
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-3">
                                        <span x-show="student.gender === 'L'"
                                            class="inline-flex items-center justify-center w-6 h-6 rounded-sm bg-blue-50 text-blue-600 text-[10px] font-bold">L</span>
                                        <span x-show="student.gender === 'P'"
                                            class="inline-flex items-center justify-center w-6 h-6 rounded-sm bg-pink-50 text-pink-500 text-[10px] font-bold">P</span>
                                    </td>
                                </tr>
                            </template>
                        </tbody>
                ...
                <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex justify-end shrink-0">
                    <button @click="modalOpen = false"
                        class="px-5 py-2.5 text-[13px] font-bold text-gray-600 hover:text-gray-900 bg-white border border-gray-200 hover:bg-gray-50 rounded-sm transition-colors">Tutup</button>
                </div>
            </div>
        </div>
    </div>
