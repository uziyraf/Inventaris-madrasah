...
@section('content')
    ...
    <div id="modalImport" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50">
        <div class="bg-white rounded-sm w-full max-w-lg p-8 shadow-xl">
            <h3 class="text-lg font-bold mb-2 text-gray-800">Import Data Murid</h3>
            <p class="text-sm text-gray-500 mb-6">Pilih file Excel (.xlsx) yang sudah sesuai dengan template. No Induk yang sudah ada akan otomatis diperbarui.</p>
            
            <form action="{{ route('murid.import') }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-4">
                @csrf
                <div class="mb-2">
                    <a href="{{ route('murid.template') }}" class="text-[#1c7b5b] hover:text-[#155d44] font-semibold text-sm flex items-center gap-2">
